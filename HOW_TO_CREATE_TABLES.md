...
This visual requires a **single flat table** containing both node and edge information loaded into your Power BI semantic model. This table can be generated by joining Dataflow, Dataset, and Report metadata obtained from the Power BI REST API using Power Query M, or defined manually for testing.
...
|---|---|---|---|
| `sourceId` | Text | Yes* | Globally unique ID for the upstream node (e.g. `df_wsId_objectId`). *Required only if it is a source node |
| `sourceName` | Text | No | Display name of the upstream item |
| `sourceType` | Text | No | Exactly: `Dataflow`, `Dataset`, or `Report` |
| `sourceWs` | Text | No | Workspace display name for the upstream item |
| `sourceTime` | DateTime | No | Last successful refresh timestamp for the upstream item |
| `sourceStatus` | Text | No | One of: `success`, `failed`, `progress` |
| `sourceUrl` | Text | No | Direct URL to the item in Power BI Service |
| `targetId` | Text | Yes* | Globally unique ID for the downstream item. *Required only if it's a target node |
| `targetName` | Text | No | Display name of the downstream item |
| `targetType` | Text | No | Exactly: `Dataflow`, `Dataset`, or `Report` |
| `targetWs` | Text | No | Workspace display name for the downstream item |
| `targetTime` | DateTime | No | Last successful refresh timestamp for the downstream item |
| `targetStatus` | Text | No | One of: `success`, `failed`, `progress` |
| `targetUrl` | Text | No | Direct URL to the item in Power BI Service |

> **Tip:** You only need one table. If a row represents an isolated node with no connections, populate `sourceId`, `sourceName`, `sourceType`, etc. and leave the `target` columns blank (or vice-versa). If a row represents a connection between two nodes, fill out both `source` and `target` properties in that row.

---

## 2. Power Query — REST API Approach (Advanced)
...
### Step 2 — Extracting Nodes and Edges

To build the final single flat table required by the setup, you will need to extract both **Nodes** and **Edges** using Power Query, and then **Merge** them together on the Node IDs.

Below is the M code to extract Nodes. Paste this into a new blank query (`Home → Advanced Editor`) called `Nodes`:
...
### Step 3 — Extracting the Lineage Connections (Edges)

Create a second blank query called `Edges` and paste this code:
...
### Step 4 — Merging into the Final Flat Table

Once you have both the `Nodes` and `Edges` queries:
1. Go to **Merge Queries as New**.
2. Select your `Edges` table as the first table, and your `Nodes` table as the second table.
3. Join `Edges[SourceId]` to `Nodes[NodeId]`. 
4. Expand the Nodes table to bring all the source properties (rename them to `sourceName`, `sourceType`, `sourceWs`, etc.).
5. Repeat the Merge, this time joining `Edges[TargetId]` to `Nodes[NodeId]` to bring in the target properties.
6. The resulting combined query is your **Single Flat Table** that you should load into the Visual.
...
> 
> **Important:** To use this with the visual, simply join your Nodes and Edges tables on the Target/Source IDs in Power Query to create a single flat table, exactly as described in Step 4 above, then load that table into Power BI.

---

## 4. Manual Sample Data (for testing)

Use `Enter Data` in Power BI Desktop to paste these rows directly.

### Single Lineage Table (paste into Enter Data)

| sourceId | sourceName | sourceType | sourceWs | targetId | targetName | targetType | targetWs |
|---|---|---|---|---|---|---|---|
| df_1 | Sales DF | Dataflow | WS 1 | ds_1 | Sales Dataset | Dataset | WS 1 |
| df_2 | HR DF | Dataflow | WS 1 | ds_2 | HR Dataset | Dataset | WS 1 |
| ds_1 | Sales Dataset | Dataset | WS 1 | rp_1 | Sales Report | Report | WS 1 |
| ds_1 | Sales Dataset | Dataset | WS 1 | rp_3 | Exec Summary | Report | WS 1 |
| ds_2 | HR Dataset | Dataset | WS 1 | rp_2 | HR Overview | Report | WS 1 |
| ds_2 | HR Dataset | Dataset | WS 1 | rp_3 | Exec Summary | Report | WS 1 |

---

## 5. Field Well Mapping in the Visual

After successfully loading your single table into your Power BI semantic model, map the columns to the visual:

1. Insert the **LineageVisual** onto your report canvas.
2. In the **Fields** pane, expand the table and drag your `sourceId`, `sourceName`, `sourceType`, `sourceWs` and their corresponding `target` equivalents into the visual's field wells.
3. You can also optionally map `sourceStatus`, `targetStatus`, etc. for refresh tracking.
...
## 7. Refresh Status via Refresh History API
